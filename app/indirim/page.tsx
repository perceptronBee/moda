...
export default function IndirimPage() {
  const products = PRODUCTS.filter(
    ...
    const aPct = (a.oldPrice! - a.price) / a.oldPrice!;
    const bPct = (b.oldPrice! - b.price) / b.oldPrice!;
    return bPct - aPct;
  });
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 py-6 sm:py-8 max-w-7xl mx-auto w-full">
      <nav className="flex items-center gap-2 text-sm text-[var(--color-muted)] mb-4 sm:mb-6">
        ...
        <span>/</span>
        <span className="text-[var(--color-fg)]">İndirim</span>
      </nav>

      <p className="meta mb-2 text-[var(--color-accent)]">SINIRLI SÜRELİ</p>
      <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl tracking-wide mb-2 text-[var(--color-accent)]">
        İndirim
      </h1>
      <p className="text-sm text-[var(--color-muted)] mb-8">
        {products.length} ürün · %50'ye varan indirimler
      </p>
